...
> Planning/spec document only.
> This file is web-local scope intent, not the cross-workspace current-state source of truth.
> For current code truth, use [root `STATUS.md`](../../../STATUS.md) and [root `CONTEXT.md`](../../../CONTEXT.md).
>
> Current-code delta note (2026-05-17):
> this PRD uses some target vocabulary that the live code has not fully adopted yet.
> Current code still carries `vendor`, `society_manager`, `delivery_boy`, and `service_boy`.
> Current code does not yet use `delivery_agent` or `field_technician` as the live route/role vocabulary.
> Read this file as target scope unless a current-state doc says otherwise.
...
This PRD uses a target role model of 16 person-roles, partitioned by which side of the business they belong to.
...
This PRD collapses legacy `vendor` into `supplier`. Current code still supports both role names.

## 2.4 Roles removed from v1

- This section describes the target PRD cleanup model, not the live role enum as of 2026-05-17.
...
- `society_manager` as a target role is dropped in this PRD model — but current web/mobile code still uses `society_manager` in live routing and policy vocabulary.
...
| Master | Purpose |
|--------|---------|
| **Role Master** | Defines the role catalog and permission profiles. Drives RBAC. Current code still contains transitional aliases and extra live roles beyond this PRD target model. |
...
- `docs/planning/catalog-todo.md` (catalog confirmation worksheet — for client review)
